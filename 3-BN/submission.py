...
class ExactInference:
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                posiibleX = util.colToX(col)
                possibleY = util.rowToY(row)
                distance = ((agentX - posiibleX) ** 2 + (agentY - possibleY) ** 2) ** 0.5
                self.belief.setProb(row, col, prob * util.pdf(distance, Const.SONAR_STD, observedDist))
    ...
    def elapseTime(self) -> None:
        ...
        # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
        belief_next = []
        for row_next in range(self.belief.getNumRows()):
            belief_next_row = []
            for col_next in range(self.belief.getNumCols()):
                prob = 0
                for row in range(self.belief.getNumRows()):
                    for col in range(self.belief.getNumCols()):
                        if ((row, col), (row_next, col_next)) in self.transProb.keys():
                            prob += self.belief.getProb(row, col) * self.transProb[((row, col), (row_next, col_next))]
                belief_next_row.append(prob)
            belief_next.append(belief_next_row)
        self.belief.grid = belief_next
        self.belief.normalize()
        # END_YOUR_CODE
